...
text="[==== <b>Time to xdroidAOSP A12</b> ====]" msg

# Repo init
repo init -u https://github.com/xdroid-oss/xd_manifest -b twelve --depth=1
...
if [ $BUILD_START = "1" ]; then
    text="<code>lunch xdroid_begonia-userdebug</code>" msg
    source build/envsetup.sh && lunch xdroid_begonia-userdebug
    ...
    # Upload
    FileZip=$(echo $MainPath/out/target/product/begonia/INA.XD.*.zip)
    ...
    if [ ! -e $FileZip ]; then
        ...
        printf "<code>XDROID_REVISION=$(cat out/soong.log | grep "XDROID_REVISION" | awk '{ print $5 }')</code>\n"
        printf "<code>TARGET_PRODUCT=xdroid_begonia</code>\n"
